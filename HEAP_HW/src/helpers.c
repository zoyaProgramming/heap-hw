...
void *sf_mem_grow_safe()
{
   ...
   // header_t new_hdrp = parse_header(*(uint64_t *)(hdrp)); /*gets the header ptr of the current block, previously the epilogue*/
    PUT_PSIZE(hdrp, PAGE_SZ - ALIGNMENT_SIZE);
    PUT_BSIZE(hdrp, PAGE_SZ);
    PUT_QLIST(hdrp, 0);
    PUT_ALLOC(hdrp, 0);
...
void *mem_init()
{
    ...
    void *prologue = heap_start + ALIGNMENT_SIZE; /*ptr to prologue (1 row after heap)*/
...
void * place(void * ptr, size_t asize, size_t size){
    ...
    write_hdr(HDRP(ptr), hdr);       /* write hdr*/
    write_hdr(FTRP(ptr), hdr); /*write footer*/

    if(bsize - asize >= 32){/*handle splitting*/
        void * splitptr = NEXT_BLKP(ptr);
        void * hdrp = HDRP(splitptr); void * ftrp = FTRP(splitptr);
        PUT_BSIZE(hdrp, bsize - asize);
        PUT_ALLOC(hdrp, 0);
        PUT_QLIST(hdrp, 0);
        PUT(splitptr, NEXT_BLKP(splitptr)); /*next block ptr*/
